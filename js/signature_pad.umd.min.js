...
(function (g, f) {
  ...
  var b = (d, n) => () => (d && (n = d((d = 0))), n);
  var L = (d, n) => {
      for (var t in n) y(d, t, { get: n[t], enumerable: !0 });
    },
    G = (d, n, t, e) => {
      if ((n && typeof n == "object") || typeof n == "function")
        for (let i of U(n))
          !W.call(d, i) && i !== t && y(d, i, { get: () => n[i], enumerable: !(e = D(n, i)) || e.enumerable });
      return d;
    };
  var B = (d) => G(y({}, "__esModule", { value: !0 }), d);
  var f,
    P = b(() => {
      "use strict";
      f = class {
        ...
        time;
        constructor(n, t, e, i) {
          if (isNaN(n) || isNaN(t)) throw new Error(`Point is invalid: (${n}, ${t})`);
    ...
    S = b(() => {
      ...
      x = class d {
        ...
        static fromPoints(n, t) {
          let e = this.calculateControlPoints(n[0], n[1], n[2]).c2,
        ...
        static calculateControlPoints(n, t, e) {
          let i = n.x - t.x,
            ...
            r = t.x - e.x,
            c = t.y - e.y,
            a = { x: (n.x + t.x) / 2, y: (n.y + t.y) / 2 },
            h = { x: (t.x + e.x) / 2, y: (t.y + e.y) / 2 },
            l = Math.sqrt(i * i + o * o),
            s = Math.sqrt(r * r + c * c),
            v = a.x - h.x,
            p = a.y - h.y,
            m = l + s == 0 ? 0 : s / (l + s),
            _ = { x: h.x + v * m, y: h.y + p * m },
            u = t.x - _.x,
            g = t.y - _.y;
          return { c1: new f(a.x + u, a.y + g), c2: new f(h.x + u, h.y + g) };
        }
        length() {
          let t = 0,
            e,
          ...
          for (let o = 0; o <= 10; o += 1) {
            let r = o / 10,
              c = this.point(r, this.startPoint.x, this.control1.x, this.control2.x, this.endPoint.x),
              a = this.point(r, this.startPoint.y, this.control1.y, this.control2.y, this.endPoint.y);
            if (o > 0) {
              let h = c - e,
                l = a - i;
              t += Math.sqrt(h * h + l * l);
        ...
        point(n, t, e, i, o) {
          return (
            t * (1 - n) * (1 - n) * (1 - n) + 3 * e * (1 - n) * (1 - n) * n + 3 * i * (1 - n) * n * n + o * n * n * n
          );
        }
      };
    });
  ...
  function O(d, n = 250) {
    let t = 0,
      ...
      r,
      c = () => {
        ((t = Date.now()), (e = null), (i = d.apply(o, r)), e || ((o = null), (r = [])));
      };
    return function (...h) {
      let l = Date.now(),
        s = n - (l - t);
      return (
        ...
        (r = h),
        s <= 0 || s > n
          ? (e && (clearTimeout(e), (e = null)), (t = l), (i = d.apply(o, r)), e || ((o = null), (r = [])))
          : e || (e = window.setTimeout(c, s)),
        i
      );
    };
  }
      ...
      w = class d extends E {
        constructor(t, e = {}) {
          ...
          ((this.velocityFilterWeight = e.velocityFilterWeight || 0.7),
            ...
            (this.throttle = e.throttle ?? 16),
            (this.minDistance = e.minDistance ?? 5),
            ...
            (this.canvasContextOptions = e.canvasContextOptions ?? {}),
            (this._strokeMoveUpdate = this.throttle
              ? O(d.prototype._strokeUpdate, this.throttle)
              : d.prototype._strokeUpdate),
        ...
        fromDataURL(t, e = {}) {
          return new Promise((i, o) => {
            let r = new Image(),
              c = e.ratio || window.devicePixelRatio || 1,
              a = e.width || this.canvas.width / c,
              h = e.height || this.canvas.height / c,
              l = e.xOffset || 0,
              s = e.yOffset || 0;
            (this._reset(this._getPointGroupOptions()),
              (r.onload = () => {
                (this._ctx.drawImage(r, l, s, a, h), i());
              }),
              (r.onerror = (v) => {
                o(v);
              }),
        ...
        _strokeBegin(t) {
          ...
          switch (t.event.type) {
            case "mousedown":
              (i("mousemove", this._handleMouseMove, { passive: !1 }),
                i("mouseup", this._handleMouseUp, { passive: !1 }));
              break;
            case "touchstart":
              (i("touchmove", this._handleTouchMove, { passive: !1 }),
                i("touchend", this._handleTouchEnd, { passive: !1 }),
                i("touchcancel", this._handleTouchCancel, { passive: !1 }));
              break;
            case "pointerdown":
              (i("pointermove", this._handlePointerMove, { passive: !1 }),
                i("pointerup", this._handlePointerUp, { passive: !1 }),
                i("pointercancel", this._handlePointerCancel, { passive: !1 }));
              break;
            default:
          }
          this._drawingStroke = !0;
          let o = this._getPointGroupOptions(),
            r = { ...o, points: [] };
          (this._data.push(r), this._reset(o), this._strokeUpdate(t));
        }
        _strokeUpdate(t) {
          ...
          let e = this._createPoint(t.x, t.y, t.pressure),
            ...
            o = i.points,
            r = o.length > 0 && o[o.length - 1],
            c = r ? e.distanceTo(r) <= this.minDistance : !1,
            a = this._getPointGroupOptions(i);
          if (!r || !(r && c)) {
            let h = this._addPoint(e, a);
            (r ? h && this._drawCurve(h, a) : this._drawDot(e, a),
              o.push({ time: e.time, x: e.x, y: e.y, pressure: e.pressure }));
          }
        ...
        _createPoint(t, e, i) {
          let o = this.canvas.getBoundingClientRect();
          return new f(t - o.left, e - o.top, i, new Date().getTime());
        }
        _addPoint(t, e) {
          let { _lastPoints: i } = this;
          if ((i.push(t), i.length > 2)) {
            i.length === 3 && i.unshift(i[0]);
            let o = this._calculateCurveWidths(i[1], i[2], e),
              r = x.fromPoints(i, o);
            return (i.shift(), r);
          }
          return null;
        ...
        _calculateCurveWidths(t, e, i) {
          let o = i.velocityFilterWeight * e.velocityFrom(t) + (1 - i.velocityFilterWeight) * this._lastVelocity,
            r = this._strokeWidth(o, i),
            c = { end: r, start: this._lastWidth };
          return ((this._lastVelocity = o), (this._lastWidth = r), c);
        }
        _strokeWidth(t, e) {
        ...
        _drawCurve(t, e) {
          let i = this._ctx,
            o = t.endWidth - t.startWidth,
            r = Math.ceil(t.length()) * 2;
          ...
          for (let c = 0; c < r; c += 1) {
            let a = c / r,
              ...
              p = v * s,
              m = p * t.startPoint.x;
            ((m += 3 * v * a * t.control1.x), (m += 3 * s * h * t.control2.x), (m += l * t.endPoint.x));
            let _ = p * t.startPoint.y;
            ((_ += 3 * v * a * t.control1.y), (_ += 3 * s * h * t.control2.y), (_ += l * t.endPoint.y));
            let u = Math.min(t.startWidth + l * o, e.maxWidth);
            this._drawCurveSegment(m, _, u);
          }
          (i.closePath(), i.fill());
        }
        _drawDot(t, e) {
          let i = this._ctx,
            o = e.dotSize > 0 ? e.dotSize : (e.minWidth + e.maxWidth) / 2;
          (i.beginPath(), this._drawCurveSegment(t.x, t.y, o), i.closePath(), (i.fillStyle = e.penColor), i.fill());
        }
        _fromData(t, e, i) {
          for (let o of t) {
            let { points: r } = o,
            ...
            if (r.length > 1)
              for (let a = 0; a < r.length; a += 1) {
                let h = r[a],
                  l = new f(h.x, h.y, h.pressure, h.time);
                a === 0 && this._reset(c);
                let s = this._addPoint(l, c);
                s && e(s, c);
              }
            else (this._reset(c), i(r[0], c));
          }
        }
        toSVG({ includeBackgroundColor: t = !1, includeDataUrl: e = !1 } = {}) {
          let i = this._data,
            ...
            r = 0,
            c = 0,
            a = this.canvas.width / o,
            h = this.canvas.height / o,
          ...
          if (
            ...
            l.setAttribute("viewBox", `${r} ${c} ${a} ${h}`),
            l.setAttribute("width", a.toString()),
            l.setAttribute("height", h.toString()),
            t && this.backgroundColor)
          ) {
            let s = document.createElement("rect");
            (s.setAttribute("width", "100%"),
              s.setAttribute("height", "100%"),
              s.setAttribute("fill", this.backgroundColor),
              l.appendChild(s));
          }
          if (e && this._dataUrl) {
            let s = this._dataUrlOptions?.ratio || window.devicePixelRatio || 1,
              v = this._dataUrlOptions?.width || this.canvas.width / s,
              p = this._dataUrlOptions?.height || this.canvas.height / s,
              m = this._dataUrlOptions?.xOffset || 0,
              _ = this._dataUrlOptions?.yOffset || 0,
              u = document.createElement("image");
            (u.setAttribute("x", m.toString()),
              u.setAttribute("y", _.toString()),
              u.setAttribute("width", v.toString()),
              u.setAttribute("height", p.toString()),
              u.setAttribute("preserveAspectRatio", "none"),
              u.setAttribute("href", this._dataUrl),
              l.appendChild(u));
          }
          return (
            this._fromData(
              ...
              (s, { penColor: v }) => {
                ...
                if (!isNaN(s.control1.x) && !isNaN(s.control1.y) && !isNaN(s.control2.x) && !isNaN(s.control2.y)) {
                  let m = `M ${s.startPoint.x.toFixed(3)},${s.startPoint.y.toFixed(3)} C ${s.control1.x.toFixed(3)},${s.control1.y.toFixed(3)} ${s.control2.x.toFixed(3)},${s.control2.y.toFixed(3)} ${s.endPoint.x.toFixed(3)},${s.endPoint.y.toFixed(3)}`;
                  (p.setAttribute("d", m),
                    p.setAttribute("stroke-width", (s.endWidth * 2.25).toFixed(3)),
                    p.setAttribute("stroke", v),
                    p.setAttribute("fill", "none"),
                    p.setAttribute("stroke-linecap", "round"),
                    l.appendChild(p));
                }
              },
              (s, { penColor: v, dotSize: p, minWidth: m, maxWidth: _ }) => {
                let u = document.createElement("circle"),
                  g = p > 0 ? p : (m + _) / 2;
                (u.setAttribute("r", g.toString()),
                  u.setAttribute("cx", s.x.toString()),
                  u.setAttribute("cy", s.y.toString()),
                  u.setAttribute("fill", v),
                  l.appendChild(u));
              },
            ),
            l.outerHTML
